...
EngineOutput Engine::ingest(const InputEnvelope& input,
                            std::uint64_t now_ms) {
  EngineOutput output;
  ...
  if (!game.cursor.empty() && input.cursor <= game.cursor) {
    trace(output, input.cursor == game.cursor ? "INPUT_DUPLICATE"
  ...
  const bool must_bootstrap = input.update_mode == UpdateMode::Bootstrap ||
                              !game.bootstrapped;
  if (must_bootstrap) {
  ...
  switch (sequence_state_) {
    case SequenceState::LeadIn:
      if (now_ms >= lead_due_ms_) {
        ...
        emit_motion(CommandType::MotionExtend, kMaxStrokeMm,
                    motion_deadline_ms_, output);
    ...
    case SequenceState::Raised:
      if (now_ms >= raised_due_ms_) {
        ...
        emit_motion(CommandType::MotionRetract, 0, motion_deadline_ms_,
                    output);
...
EngineOutput Engine::report_position(std::int32_t position_mm,
  ...
  } else if (sequence_state_ == SequenceState::Retracting &&
             position_mm == 0) {
...
bool Engine::validate_input(const InputEnvelope& input,
  ...
  if (input.game_pk <= 0 || input.cursor.empty() ||
      (input.game_number != 1 && input.game_number != 2) ||
      input.inning < 0 || input.outs < 0 || input.outs > 3 ||
      input.away_runs < 0 || input.home_runs < 0) {
...
bool Engine::persist_once(std::string_view event_key,
                          EngineOutput& output) {
...
bool Engine::seed_bootstrap(const InputEnvelope& input,
                            EngineOutput& output) {
  for (const auto& play : input.plays) {
    if (!play.complete || play.event_key.empty() ||
    ...
    if (review == ReviewState::Pending ||
        review == ReviewState::Overturned) {
      continue;
    }
    const auto lookup = ledger_.lookup(play.event_key);
    if (lookup == LedgerLookup::Error ||
        (lookup == LedgerLookup::Missing &&
         !ledger_.persist(play.event_key))) {
...
void Engine::consider_play(const InputEnvelope& input,
                           const PlayEvidence& play,
  ...
  enqueue(CelebrationRequest{play.event_key, CelebrationKind::HomeRun,
                             play.batter_name},
          now_ms, output);
}
...
void Engine::consider_final(const InputEnvelope& input, Phase previous_phase,
  ...
  enqueue(CelebrationRequest{event_key, CelebrationKind::MetsWin,
                             "Mets Win!"},
          now_ms, output);
}
...
void Engine::start_next(std::uint64_t now_ms, EngineOutput& output) {
  ...
  output.commands.push_back(Command{CommandType::DisplayRender,
                                    active_.event_key,
                                    active_.kind,
                                    active_.subject,
                                    0,
                                    0});
  output.commands.push_back(Command{CommandType::LedCelebrate,
                                    active_.event_key,
                                    active_.kind,
                                    active_.subject,
                                    0,
...
void Engine::latch_fault(std::string_view reason, EngineOutput& output) {
  ...
  queue_.clear();
  output.commands.push_back(Command{CommandType::MotionDisable,
                                    active_.event_key,
                                    active_.kind,
                                    active_.subject,
                                    0,
...
void Engine::emit_motion(CommandType type, std::int32_t position_mm,
                         ...
                         EngineOutput& output) const {
  output.commands.push_back(Command{type,
                                    active_.event_key,
                                    active_.kind,
                                    active_.subject,
                                    position_mm,
