...
const apple::core::CoreEvent *event_at(const CoreHandle *handle,
                                       std::size_t index) {
  if (handle == nullptr || index >= handle->output.events.size()) {
    return nullptr;
  }
  return &handle->output.events[index];
}
...
APPLE_EXPORT int apple_core_begin_input(void *raw, int schema_version,
                                        int update_mode, int game_pk,
                                        int game_number, const char *cursor,
                                        int phase, int half, int inning,
                                        int outs, int away_team_id,
                                        int home_team_id, int away_runs,
                                        int home_runs) {
  ...
  if (!enum_in_range(update_mode, 1, input.update_mode) ||
      !enum_in_range(phase, 5, input.phase) ||
...
APPLE_EXPORT int apple_core_add_play(void *raw, const char *event_key,
  ...
  if (!enum_in_range(play_kind, 2, play.kind) ||
...
APPLE_EXPORT int apple_core_commit_input(void *raw, double now_ms) {
  auto *handle = as_handle(raw);
  if (handle == nullptr || !handle->pending_ready || now_ms < 0)
    return 0;
  handle->output = handle->engine.ingest(handle->pending,
...
APPLE_EXPORT void apple_core_set_ledger_failures(void *raw, int reads,
  ...
  auto *handle = as_handle(raw);
  if (handle != nullptr)
    handle->ledger.set_failures(reads != 0, writes != 0);
}
...
APPLE_EXPORT int apple_core_event_count(const void *raw) {
  const auto *handle = as_handle(raw);
  return handle == nullptr ? 0
                           : static_cast<int>(handle->output.events.size());
}

APPLE_EXPORT int apple_core_event_type(const void *raw, int index) {
  const auto *event = index < 0 ? nullptr : event_at(as_handle(raw), index);
  return event == nullptr ? -1 : static_cast<int>(event->type);
}

APPLE_EXPORT const char *apple_core_event_key(const void *raw, int index) {
  const auto *event = index < 0 ? nullptr : event_at(as_handle(raw), index);
  return event == nullptr ? "" : event->event_key.c_str();
}

APPLE_EXPORT int apple_core_event_celebration(const void *raw, int index) {
  const auto *event = index < 0 ? nullptr : event_at(as_handle(raw), index);
  return event == nullptr ? -1 : static_cast<int>(event->celebration);
}

APPLE_EXPORT const char *apple_core_event_subject(const void *raw, int index) {
  const auto *event = index < 0 ? nullptr : event_at(as_handle(raw), index);
  return event == nullptr ? "" : event->subject.c_str();
}
...
APPLE_EXPORT int apple_core_command_type(const void *raw, int index) {
  const auto *command = index < 0 ? nullptr : command_at(as_handle(raw), index);
...
APPLE_EXPORT double apple_core_command_deadline_ms(const void *raw, int index) {
  const auto *command = index < 0 ? nullptr : command_at(as_handle(raw), index);
  return command == nullptr ? 0.0 : static_cast<double>(command->deadline_ms);
}

APPLE_EXPORT int apple_core_trace_count(const void *raw) {
  const auto *handle = as_handle(raw);
  return handle == nullptr ? 0 : static_cast<int>(handle->output.traces.size());
}
