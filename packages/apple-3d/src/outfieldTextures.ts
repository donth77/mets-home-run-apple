...
export function createOutfieldTextures() {
  const grass = canvasTexture(1024, 1024, [5, 4], (context, width, height) => {
    ...
    for (let mark = 0; mark < 10500; mark += 1) {
  ...
  const track = canvasTexture(1024, 1024, [7, 2], (context, width, height) => {
    ...
    context.fillRect(0, 0, width, height);
    for (let grain = 0; grain < 9400; grain += 1) {
  ...
  const wall = canvasTexture(2048, 256, [1, 1], (context, width, height) => {
  ...
  const concrete = canvasTexture(512, 512, [4, 3], (context, width, height) => {
    context.fillStyle = "#4a504f";
    context.fillRect(0, 0, width, height);
    for (let fleck = 0; fleck < 1800; fleck += 1) {
  ...
  const louvers = canvasTexture(1024, 512, [1, 1], (context, width, height) => {
    const gradient = context.createLinearGradient(0, 0, 0, height);
